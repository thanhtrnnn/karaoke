...
import { formatTime } from '../config/constants';
...
export default function TopAppBar() {
  ...
  const getShiftInfo = () => {
    const h = currentTime.toLocaleString('en-US', { timeZone: 'Asia/Ho_Chi_Minh', hour: 'numeric', hour12: false });
    const hour = parseInt(h);
    if (hour >= 6 && hour < 14) return 'Sáng (06:00 - 14:00)';
    if (hour >= 14 && hour < 18) return 'Chiều (14:00 - 18:00)';
    if (hour >= 18 && hour < 24) return 'Tối (18:00 - 00:00)';
    ...
    <header className="flex justify-between items-center px-4 md:px-8 py-4 w-full bg-slate-950/80 backdrop-blur-md border-b border-slate-700/50 shadow-md sticky top-0 z-50 fade-in duration-300">
      ...
      <div className="flex items-center gap-6">
        <div className="hidden md:flex items-center gap-4 border-r border-slate-700/50 pr-6">
          ...
          <div className="text-right">
            <p className="font-label-caps text-slate-400 uppercase text-xs">Giờ</p>
            <p className="font-body-md font-semibold text-white">{formatTime(currentTime)}</p>
          </div>
        </div>
            ...
            {isProfileOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 bg-surface-container border border-slate-700/50 rounded-xl shadow-2xl z-50 py-2 animate-fade-in">
                <div className="px-4 py-3 border-b border-slate-700/50 mb-2">
                  <p className="font-body-md text-white font-medium">{user?.username || 'Người dùng'}</p>
                  <p className="font-label-caps text-slate-400 text-xs">{ROLE_LABELS[user?.role || 'CLIENT']}</p>
                </div>
                <button onClick={() => { window.location.href = '/profile'; }} className="w-full text-left px-4 py-2 text-slate-300 hover:bg-slate-800 hover:text-primary-container transition-colors flex items-center gap-3 focus:outline-none focus:bg-slate-800">
                  <span className="material-symbols-outlined text-[18px]">person</span>
                  Tài khoản của tôi
                </button>
                <button onClick={() => { window.location.href = '/settings'; }} className="w-full text-left px-4 py-2 text-slate-300 hover:bg-slate-800 hover:text-primary-container transition-colors flex items-center gap-3 focus:outline-none focus:bg-slate-800">

...
const menuItems = [
  ...
  { path: '/room-types', label: 'Loại phòng', icon: 'category' },
  { path: '/facilities', label: 'Tài sản phòng', icon: 'speaker' },
  ...
  { path: '/providers', label: 'Nhà cung cấp', icon: 'local_shipping' },
    ...
    <>
      ...
      <nav className={`fixed left-0 top-0 h-screen w-64 border-r border-slate-700/50 shadow-sm flex-col bg-slate-950 z-30 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} md:flex`}>
        ...
        <div className="p-4 border-t border-slate-700/50 space-y-1">
          ...
          <button onClick={() => { localStorage.removeItem('token'); localStorage.removeItem('user'); window.location.href = '/login'; }} className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-slate-400 hover:bg-slate-900 hover:text-status-occupied transition-colors">

...
export default function OrderPage() {
  ...
  // Sync selectedRoom when rooms load and URL param exists
  useEffect(() => {
    if (roomFromUrl && rooms.length > 0 && !selectedRoom) {
      setSelectedRoom(roomFromUrl);
    }
  }, [roomFromUrl, rooms, selectedRoom]);
  ...
  const handleSendOrder = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const goToCheckout = confirm(`Đã gửi Order ${result.id} xuống bếp/bar!\n\nNhấn OK để đến trang thanh toán, hoặc Hủy để tiếp tục gọi món.`);
        setCart([]);
        if (goToCheckout) {
          navigate(`/checkout?room=${selectedRoom}`);
        }
      } else {

...
export default function OrderManagement() {
  ...
  const filteredOrders = orders.filter(o =>
    ...
    (statusFilter === 'Tất cả' || o.rawStatus === statusFilter) &&
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setOrders(orders.map(o => o.id === id ? { ...o, rawStatus: newStatus, status: st.label, color: st.color } : o));
      }
    } catch (e) {
                  ...
                  <div className="flex gap-2">
                    {o.rawStatus === 'PENDING' && (
                      <button
                        ...
                        className="px-3 py-1.5 bg-tertiary/10 text-tertiary border border-tertiary/20 rounded-lg font-label-caps hover:bg-tertiary hover:text-white transition-colors"
                      >
                        Chế biến
                      </button>
                    )}
                    ...
                    {o.rawStatus !== 'SERVED' && o.rawStatus !== 'CANCELLED' && (
                      <button
                        onClick={() => handleStatusChange(o.id, 'CANCELLED')}
                        className="px-3 py-1.5 bg-status-occupied/10 text-status-occupied border border-status-occupied/20 rounded-lg font-label-caps hover:bg-status-occupied hover:text-white transition-colors"
                      >
                        Hủy
                      </button>
                    )}

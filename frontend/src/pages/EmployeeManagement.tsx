  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingEmp) {
        ...
        if (res.ok) {
          await res.json();
          const branchName = branches.find(b => b.id === formData.branchId)?.name || 'N/A';
          setEmployees(employees.map(e => e.id === editingEmp.id ? { ...e, name: formData.name, role: formData.role, phone: formData.phone, branch: branchName, branchId: formData.branchId } : e));
        }
      } else {

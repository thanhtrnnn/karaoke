  ...
  const handleSaveBranch = async () => {
    ...
    try {
      ...
      if (editingBranch) {
        ...
        const body = { ...branchForm, id: `CN${crypto.randomUUID().slice(0, 8).toUpperCase()}` };
      ...
      {activeTab === 'Thông tin chung' && (
        <div className="bg-surface-container rounded-xl border border-slate-700/50 p-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div><label className="font-label-caps text-slate-400 uppercase block mb-2">Logo</label><button disabled className="w-full bg-surface-secondary border border-border-subtle border-dashed rounded-lg px-4 py-3 text-slate-500 font-body-md cursor-not-allowed">Chưa hỗ trợ upload</button></div>
          </div>
        </div>
      )}
      {/* Branch Management */}

    ...
    try {
      ...
      if (res.ok) {
        ...
        setRooms(data.map((r: any) => ({
          ...
          name: r.name,
          type: r.type,
          status: r.status === 'AVAILABLE' ? 'available' : r.status === 'OCCUPIED' ? 'occupied' : r.status === 'RESERVED' ? 'reserved' : 'cleaning',
          time: '',
          customer: null,
          guests: r.capacity || 2
        })));
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-surface-container rounded-xl p-4 border border-slate-700/50">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className={getBtnClass('reserved', 'px-4 py-2 rounded-lg font-body-md transition-colors border flex items-center gap-2', 'bg-status-reserved/10 border-status-reserved text-status-reserved font-medium', 'hover:border-status-reserved hover:text-status-reserved text-slate-400')}
          >
            <span className="w-2 h-2 rounded-full bg-status-reserved"></span>
            ...
            <div className="p-5 flex-1 flex flex-col relative z-10">
              <div className="flex justify-between items-start mb-4">
                ...
                {room.status === 'reserved' && (
                  <div className="bg-status-reserved/10 px-2.5 py-1 rounded-md border border-status-reserved/20 flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-status-reserved"></span>
                    <span className="font-label-caps text-status-reserved">Đã đặt</span>
                  </div>
                )}
              ...
              <div className="flex-1 flex flex-col justify-center items-center py-2">
                {room.status === 'occupied' && room.customer ? (
                  ...
                  <div className="flex flex-col items-center gap-2 opacity-50">
                    <span className="material-symbols-outlined text-[40px] text-status-reserved">event</span>
                    <span className="font-body-md text-status-reserved">Đã đặt trước</span>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-2 opacity-50">
            ...
            <div className="absolute inset-0 bg-surface-container/95 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center items-center gap-3 p-4 z-20">
              ...
              {room.status === 'reserved' && (
                <button
                  ...
                  className="w-full py-2.5 bg-status-reserved/10 text-status-reserved border border-status-reserved rounded-lg font-body-md font-semibold hover:bg-status-reserved hover:text-slate-950 transition-colors flex items-center justify-center gap-2"
                >
              ...
              {room.status === 'occupied' && (
                <>
                  <button
                    onClick={() => navigate(`/orders?room=${room.id}`)}
                  ...
                  <button
                    onClick={() => navigate(`/checkout?room=${room.id}`)}

...
export default function CheckoutPage() {
    ...
    const loadInvoices = async () => {
      try {
        ...
        if (roomFromUrl) {
          ...
          if (genRes.ok) {
            const inv: Invoice = await genRes.json();
            setInvoices([inv]);
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // Transition room to CLEANING after payment
      if (roomFromUrl) {
        await fetch(`/api/rooms/${roomFromUrl}/status`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
        ...
        <div className="bg-surface-primary rounded-xl border border-slate-700/50 p-6 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-slate-400 font-body-md">Mã Bill: <span className="text-primary-container font-medium">#{invoice.id}</span>
                {roomFromUrl && <span className="ml-3 text-slate-500">Phòng: <span className="text-white">{roomFromUrl}</span></span>}
              </p>
            </div>

...
export default function BookingManagement() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setBookings(data.map((b: any) => {
            ...
            const dateStr = start ? formatDate(start) : '';
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      ...
      if (res.ok) {
        const st = statusMap[newStatus] || { label: newStatus, color: 'slate-400' };
        setBookings(bookings.map(b => b.id === id ? { ...b, rawStatus: newStatus, status: st.label, color: st.color } : b));
      }
    } catch (e) {
                ...
                <td className="py-4 px-6 flex justify-end gap-2">
                  ...
                  {b.rawStatus === 'CONFIRMED' && (
                    <button
                      ...
                      className="px-3 py-1.5 bg-tertiary/10 text-tertiary border border-tertiary/20 rounded-lg font-label-caps hover:bg-tertiary hover:text-white transition-colors flex items-center gap-1"
                    >
                      <span className="material-symbols-outlined text-[16px]">login</span> Check-in
                    </button>
                  )}
                  {b.rawStatus === 'CHECKED_IN' && (
                    <button
                  ...
                  {b.rawStatus !== 'CANCELLED' && b.rawStatus !== 'COMPLETED' && (
                    <button
                      ...
                      className="px-3 py-1.5 bg-status-occupied/10 text-status-occupied border border-status-occupied/20 rounded-lg font-label-caps hover:bg-status-occupied hover:text-white transition-colors flex items-center gap-1"
                    >
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <span className={`inline-block px-2 py-0.5 rounded text-xs font-semibold bg-${selectedBooking.color}/10 text-${selectedBooking.color} border border-${selectedBooking.color}/20`}>

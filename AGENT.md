...
## 9. TESTING STATUS (DO NOT DELETE — pick up from here)

Plan file: `/Users/quant/.claude/plans/gentle-sparking-hopcroft.md`

### Phase 1: Backend Tests — 95% done, 3 failures remain

**Done (all green):**
- `backend/src/test/resources/application-test.properties`
- 6 repository tests: `ServiceOrderRepositoryTest`, `UserAccountRepositoryTest`, `BookingRepositoryTest`, `RoomRepositoryTest`, `MenuItemRepositoryTest`, `CustomerRepositoryTest`
- `AuthControllerTest` (register/login/changePassword)
- `BookingControllerTest` (booking lifecycle + room sync)
- `OrderControllerTest` (stock decrement + status transitions)
- `ReportControllerTest` (revenue from orders)
- `InvoiceControllerTest` (generate + pay)
- `HealthControllerTest` (public health endpoint)
- `TokenAuthenticationFilterTest` (dev-token auth)
- `SecurityConfigTest` (public vs protected)
- `ApiExceptionHandlerTest` (error responses)

**FAILING — 3 tests in `CrudControllersTest.java`:**
- `branch_crud` (line 54): POST `/api/branches` returns 400
- `room_crudAndPatchStatus` (line 117): POST `/api/rooms` returns 400
- `employee_crud` (line 160): POST `/api/employees` returns 400

**Root cause investigation so far:**
- All 3 entities use Lombok `@Data @NoArgsConstructor @AllArgsConstructor`
- Controllers use `@RequestBody Entity` directly (no DTO, no `@Valid`)
- No validation annotations exist on any entity
- `@AllArgsConstructor` constructor order: `Branch(id, name, address, phone, active, rooms)`, `Room(id, name, type, capacity, hourlyPrice, status, branch, active)`, `Employee(id, fullName, phone, role, branch, active)`
- Jackson should use `@NoArgsConstructor` + setters, but something in Spring Boot 4.0 / Jackson 2.x is causing 400 on deserialization
- The 400 body is empty (no error message), suggesting a Jackson deserialization failure caught by Spring before reaching the controller
- **Try next:** Check if `boolean active` primitive vs `Boolean active` wrapper is the issue. Also check if `@JsonIgnore` on `rooms` field interferes with deserialization. Try adding `"active":true,"rooms":null` to the POST JSON. Or check if the `@AllArgsConstructor` constructor with `boolean` primitive causes Jackson to prefer it over `@NoArgsConstructor`.

### Phase 2: Frontend Tests — NOT STARTED
...
- `frontend/src/config/rbac.test.ts` — RBAC logic (CRITICAL)
- `frontend/src/components/ProtectedRoute.test.tsx` — route guard
- `frontend/src/pages/LoginPage.test.tsx`
- `frontend/src/pages/CheckoutPage.test.tsx`
- `frontend/src/pages/CustomerPage.test.tsx`
- `frontend/src/pages/ReportsPage.test.tsx`
- `frontend/src/pages/OrderPage.test.tsx`

**Key notes:**
- Frontend RBAC is frontend-only (backend has no `@PreAuthorize`)
- Components use `fetch` in `useEffect` — mock with `vi.fn()`
- Wrap components in `MemoryRouter` for routing tests
- localStorage stores user object — jsdom provides it

### Phase 3: E2E Scripts — NOT STARTED

**Files to create in `tests/e2e/`:**
- `helpers.sh` — BASE_URL, tokens, helper functions
- `00_health.sh` through `07_error_handling.sh`
- `99_full_flow.sh` — complete business flow

**Requires:** Docker stack running (`docker compose up -d`)

### Phase 4: Git History Rewrite — NOT STARTED

Plan is fully written in the plan file (Section "Phase 4"). Summary:
- Backup all branches + tag
- Create orphan branch `clean-history`
- 37 commits across 5 authors (thanhtrnnn 33%, Trung/Khanh/Tuan/Hanh 17% each)
- Verify `git diff master clean-history` is empty
- Force push only with user confirmation
